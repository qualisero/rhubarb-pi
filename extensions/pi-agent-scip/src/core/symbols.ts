...
export function parseScipSymbol(symbol: string): ParsedSymbol {
...
export function roleIsDefinition(role: number): boolean {
  return (role & scip.SymbolRole.Definition) !== 0;
}

export function roleDescription(role: number): string {
  const parts: string[] = [];
  if (role & scip.SymbolRole.Definition) parts.push('definition');
  if (role & scip.SymbolRole.Import) parts.push('import');
  if (role & scip.SymbolRole.WriteAccess) parts.push('write');
  if (role & scip.SymbolRole.ReadAccess) parts.push('read');
  if (role & scip.SymbolRole.Generated) parts.push('generated');
  if (role & scip.SymbolRole.Test) parts.push('test');
  if (role & scip.SymbolRole.ForwardDefinition) parts.push('forward');
...
function inferKindFromDescriptor(descriptor: string): string {
  if (descriptor.includes('#')) return 'Method';
  if (descriptor.includes('.(')) return 'Parameter';
  if (descriptor.endsWith('.')) return 'Function';
  return 'Symbol';
}
